...
const getInfo = (arr) => {
  let coursesName = arr.map(course => course.course);
  let studentsName = arr.reduce((acc, course) => acc.concat(course.Students), []);
  ...
  return courses.reduce((acc, course) => {
    course.Students.forEach(student => {
      if (arr.includes(student)) {
        acc.push({ Student: student, course: course.course });
      }
    });
    return acc;
  }, []);
